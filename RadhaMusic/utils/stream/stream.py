...
import config
from RadhaMusic import Carbon, YouTube, app, YTB
...
from RadhaMusic.utils.pastebin import KrishBin
...
async def stream(
    ...
    if not result:
        return
    if forceplay:
        await Krish.force_stop_stream(chat_id)
    if streamtype == "playlist":
        ...
        for search in result:
            ...
            if await is_active_chat(chat_id):
                ...
                if len(check) > config.QUEUE_LIMIT:
                    return await app.send_message(chat_id, text=_["queue_9"])
                await put_queue(
                    chat_id,
                    original_chat_id,
                    f"vid_{vidid}",
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await Krish.join_call(chat_id, original_chat_id, file_path, video=None)
            await put_queue(
                chat_id,
                original_chat_id,
                file_path,
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await Krish.join_call(chat_id, original_chat_id, file_path, video=status)
            await put_queue(
                chat_id,
                original_chat_id,
                file_path,
